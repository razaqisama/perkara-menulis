        ...
        <div className={`relative h-full ${isExtended ? "w-[240px]" : "w-[60px]" } border-r-[1px] flex flex-col transition-[width] duration-300 ease-in-out`}>
            ...
            <div className="flex flex-col flex-auto overflow-y-auto overflow-x-hidden">
            {
                menuDummy.map((item, index) => {
                    return (
                        <div className="flex flex-col space-y-2 border-b-[1px] py-4">
                            <p className={`${isExtended ? "" : "opacity-0"} px-2 font-bold tracking-[0.1em]`}>Category</p>
                            {
                                subMenuDummy.map((item, index) => {
                                    return (
                                        <div className={`w-full py-2 flex flex-row overflow-hidden select-none cursor-pointer hover:bg-gray-700 transition duration-300 ease-in-out`}>
                                            ...
                                            <p className={`${isExtended ? "" : "opacity-0"} transition duration-300 ease-in-out self-end px-4 font-bold min-w-[200px] text-xs`}>MENU NAME NIME NUME NEME NOME</p>
                                        </div>
                                    )
                                }) 
                            }
